...
// ─── Resources ────────────────────────────────────────────────
server.resource("registry", "repos://registry", { description: "Cadastro mestre de repositórios acadêmicos brasileiros" }, async () => ({
    contents: [{
            uri: "repos://registry",
...
server.resource("stats", "repos://stats", { description: "Estatísticas agregadas do cadastro de repositórios" }, async () => ({
    contents: [{
            uri: "repos://stats",
            mimeType: "application/json",
...
// ─── Tools ────────────────────────────────────────────────────
server.tool("search_repository", "Busca artigos/teses em repositórios acadêmicos brasileiros. Usa OAI-PMH, DSpace REST ou scraping conforme disponibilidade.", {
    query: z.string().describe("Termo(s) de busca (ex: 'bebida fermentada')"),
    repositories: z.array(z.string()).optional()
        .describe("IDs dos repos (ex: ['BR-FED-0001']). Omita para buscar em todos os ativos."),
    ...
    for (const repo of repos) {
        try {
            ...
            errors.push(`${repo.id} (${repo.institution.acronym}): ${error.message}`);
        }
    }
    const summary = [
        ...
        ...allResults.map((r, i) => [
            ...
            r.date ? `**Data:** ${r.date}` : "",
            `**Repositório:** ${r.repositoryName} (${r.repositoryId})`,
            `**URL:** ${r.url}`,
...
server.tool("get_record_metadata", "Obtém metadados completos (Dublin Core) de um registro específico em um repositório.", {
...
server.tool("harvest_records", "Coleta registros em massa via OAI-PMH de um repositório específico. Ideal para harvesting completo ou por coleção.", {
...
server.tool("validate_repository", "Verifica a saúde e capacidades de um ou mais repositórios (conectividade, OAI-PMH, REST API).", {
    ...
    const repos = params.repositoryId
        ? [registry.getById(params.repositoryId)].filter(Boolean)
        : registry.getActive();
    if (repos.length === 0) {
    ...
    for (const repo of repos) {
        ...
        if (params.checks.includes("connectivity")) {
            ...
            try {
                ...
                checks.push(`❌ Conectividade: ${e.message}`);
            }
        }
        // OAI-PMH
