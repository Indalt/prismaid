...
export class HtmlScraper {
     ...
     */
    async search(repoUrl, repoId, repoName, query, maxResults = 50) {
     ...
     */
    parseSearchResults(html, baseUrl, repoId, repoName, query) {
        const $ = cheerio.load(html);
        const results = [];
        const seen = new Set();
        $("a").each((_, el) => {
            ...
            if (href.includes("?") || href.includes("sort_by") || href.includes("filtername"))
                return;
