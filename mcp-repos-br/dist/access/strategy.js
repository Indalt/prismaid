...
export class AccessStrategy {
    oai = new OaiPmhClient();
    rest = new DSpaceRestClient();
    scraper = new HtmlScraper();
    bdtd = new BdtdAdapter();
    scielo = new ScieloAdapter();
    usp = new UspAdapter();
    capabilities = new Map();
    /**
     * Search a repository using the best available method.
     */
    async search(repo, query, opts) {
        ...
        if (repo.id === "BR-AGG-0001" || repo.repository.url.includes("bdtd.ibict.br")) {
            try {
                return await this.bdtd.search(query, { limit: maxResults });
            }
            catch {
        ...
        if (repo.id === "BR-AGG-0002" || repo.repository.url.includes("scielo.br")) {
            try {
                return await this.scielo.search(query, { limit: maxResults });
            }
            catch {
                // Fall through
            }
        ...
        if (repo.repository.url.includes("repositorio.usp.br")) {
            try {
                const results = await this.usp.search(query, { limit: maxResults });
                if (results.length > 0)
                    return results;
            }
            catch {
        ...
        if (cap.restApiWorks) {
            try {
                ...
                if (result.items.length > 0) {
                    return result.items.map((item) => this.dspaceItemToSearchResult(item, repo));
                }
            }
            catch {
                // Fall through
        ...
        try {
            return await this.scraper.search(repo.repository.url, repo.id, repo.repository.name, query, maxResults);
        }
        catch {
            return [];
        }
     ...
     */
    async getMetadata(repo, identifier) {
        // BDTD-specific
        if (repo.id === "BR-AGG-0001" || repo.repository.url.includes("bdtd.ibict.br")) {
            try {
        ...
        if (repo.id === "BR-AGG-0002" || repo.repository.url.includes("scielo.br")) {
            try {
                return await this.scielo.getArticle(identifier);
            }
            catch { /* fall through */ }
        }
        // USP-specific
        ...
        if (cap.oaiPmhWorks && cap.oaiPmhEndpoint) {
            try {
                const record = await this.oai.getRecord(cap.oaiPmhEndpoint, identifier);
     ...
     */
    async findPdfUrl(repo, identifier) {
        const cap = await this.detectCapabilities(repo);
        // DSpace REST: get bitstreams
        if (cap.restApiWorks && identifier.length === 36) {
            // UUID format
            try {
                const bitstreams = await this.rest.getBitstreams(repo.repository.url, identifier);
                const pdf = bitstreams.find((b) => b.mimeType === "application/pdf" || b.name.toLowerCase().endsWith(".pdf"));
                if (pdf)
                    return pdf.retrieveLink;
            }
            catch {
                // Fall through
            }
     ...
     */
    oaiRecordToSearchResult(record, repo) {
     ...
     */
    dspaceItemToSearchResult(item, repo) {
        const metadata = item.metadata;
        const getFirst = (key) => metadata?.[key]?.[0]?.value ?? "";
        const getAll = (key) => metadata?.[key]?.map((v) => v.value) ?? [];
        ...
        return {
            ...
            title: getFirst("dc.title") || item.name || "",
