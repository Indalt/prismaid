...
function findRepo(target) {
    if (target.id) {
        return registryData.find((r) => r.id === target.id) ?? null;
    }
    if (target.acronym) {
        return registryData.find((r) => r.institution.acronym?.toUpperCase() === target.acronym.toUpperCase() ||
            r.institution.name?.toUpperCase().includes(target.acronym.toUpperCase())) ?? null;
    }
    return null;
}
// ─── Test functions ──────────────────────────────────────────
...
async function testOaiPmh(endpoint) {
    try {
...
async function testDSpaceRest(baseUrl) {
    try {
...
async function testHtmlScraper(url, id, name) {
    try {
...
async function main() {
    ...
    for (const target of testTargets) {
        ...
        if (repo.access?.oaiPmh?.endpoint) {
            ...
            console.log(`    OAI-PMH: ${oaiResult.ok ? "✅" : "❌"}${oaiResult.recordCount ? ` (${oaiResult.recordCount} sample records)` : ""}`);
        }
        else {
        ...
        // 4. HTML scraper test
        const scraperResult = await testHtmlScraper(repo.repository.url, repo.id, repo.repository.name);
        result.scraper = scraperResult.ok ? "ok" : "fail";
        console.log(`    Scraper: ${scraperResult.ok ? "✅" : "❌"}${scraperResult.resultCount ? ` (${scraperResult.resultCount} results)` : ""}`);
        results.push(result);
        console.log("");
    }
    // ─── Summary ─────────────────────────────────────────────
    ...
    const icon = (s) => s === "ok" ? "✅" : s === "fail" ? "❌" : s === "n/a" ? "⬜" : "⏭";
    console.log(`${pad("Repository", 24)} ${pad("Connect", 9)} ${pad("OAI-PMH", 9)} ${pad("REST", 9)} ${pad("Scraper", 9)} ${pad("Latency", 9)}`);
    console.log("─".repeat(70));
    for (const r of results) {
        console.log(`${pad(r.name, 24)} ${pad(icon(r.connectivity), 9)} ${pad(icon(r.oaiPmh), 9)} ${pad(icon(r.restApi), 9)} ${pad(icon(r.scraper), 9)} ${r.latencyMs ? r.latencyMs + "ms" : "—"}`);
    }
    // Stats
...
//# sourceMappingURL=validate-real.js.map