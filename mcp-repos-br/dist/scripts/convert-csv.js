...
function mapInstitutionType(tipo, categoria) {
    const cat = categoria.toLowerCase();
    if (cat.includes("instituto federal") || tipo.toLowerCase().includes("instituto federal"))
        return "instituto_federal";
    if (cat.includes("pública federal"))
        return "federal";
    if (cat.includes("pública estadual"))
        return "estadual";
    if (cat.includes("comunitária"))
        return "comunitaria";
    return "privada";
}
function mapPlatform(raw) {
    const lower = raw.toLowerCase();
    if (lower.includes("dspace"))
        return "dspace";
    if (lower.includes("tede"))
        return "tede";
    if (lower.includes("ojs"))
        return "ojs";
    return "other";
}
function mapContentType(raw) {
    const lower = raw.toLowerCase();
    if (lower.includes("tese") || lower === "teses" || lower === "teses/dissertações")
        return "theses";
    if (lower.includes("artigo") || lower === "artigos")
        return "articles";
    return "mixed";
}
function guessSearchEndpoints(platform, url) {
    if (platform === "dspace") {
...
//# sourceMappingURL=convert-csv.js.map